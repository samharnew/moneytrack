...
class TestPlotting(unittest.TestCase):
    ...
    def test_mf_plot(self):
        ...
        f, ax = mt.MoneyViz.plot(mf, mt.MoneyViz.Metric.Interest, cumulative=True)
        f.autofmt_xdate()
    ...
    def test_mfc_plot(self):
        ...
        f, ax = mt.MoneyViz.plot(mfc, mt.MoneyViz.Metric.Balance, cumulative=False)
        f.savefig(os.path.join(self.sample_plt_dir, "mfc_balance.pdf"))

        f, ax = mt.MoneyViz.plot(mfc, mt.MoneyViz.Metric.Transfers, cumulative=False)
        f.savefig(os.path.join(self.sample_plt_dir, "mfc_transfers.pdf"))

        f, ax = mt.MoneyViz.plot(mfc, mt.MoneyViz.Metric.Transfers, cumulative=True)
        f.savefig(os.path.join(self.sample_plt_dir, "mfc_cum_transfers.pdf"))

        f, ax = mt.MoneyViz.plot(mfc, mt.MoneyViz.Metric.InterestRate, cumulative=False)
        f.savefig(os.path.join(self.sample_plt_dir, "mfc_interest_rate.pdf"))

        f, ax = mt.MoneyViz.plot(mfc, mt.MoneyViz.Metric.InterestRate, cumulative=True)
        f.savefig(os.path.join(self.sample_plt_dir, "mfc_cum_interest_rate.pdf"))

        f, ax = mt.MoneyViz.plot(mfc, mt.MoneyViz.Metric.Interest, cumulative=False)
